...
[CreateAssetMenu(menuName = "ScriptableObject/DiffusionProfile")]
...
public class DiffusionProfileSettings : ScriptableObject
    {
        ...
        [SerializeField] internal Vector4 worldScaleAndFilterRadiusAndThicknessRemap; // X = meters per world unit, Y = filter radius (in mm), Z = remap start, W = end - start
        [SerializeField] internal Vector4 shapeParamAndMaxScatterDist;                // RGB = S = 1 / D, A = d = RgbMax(D)
        [SerializeField] internal Vector4 transmissionTintAndFresnel0;                // RGB = color, A = fresnel0
        [SerializeField] internal Vector4 disabledTransmissionTintAndFresnel0;        // RGB = black, A = fresnel0 - For debug to remove the transmission
        [SerializeField] internal int updateCount;
