   ...
   public class HDRaytracingLightCluster
    {
        ...
        public void Initialize(HDRenderPipelineRayTracingResources renderPipelineRayTracingResources, HDRenderPipeline renderPipeline)
        {
 
            m_RenderPipelineRayTracingResources = renderPipelineRayTracingResources;
            ...
            m_NumLightsPerCell = 8;
            int bufferSize = 64 * 64 * 32 * (8 + 4);
        ...
        void BuildGPULightVolumes(Camera hdCamera, HDRayTracingLights rayTracingLights)
        {
            ...
            for (int lightIdx = 0; lightIdx < rayTracingLights.hdLightArray.Count; ++lightIdx)
            {
                ...
                if (currentLight != null)
                {
                   ...
                   // m_RenderPipeline.ReserveCookieAtlasTexture(currentLight, light, currentLight.type);
                    ...
                    //{
                    //    lightPositionRWS -= hdCamera.transform.position;
                    //}
        ...
        void EvaluateClusterVolume(Camera hdCamera)
        {
            // var settings = hdCamera.GetComponent<LightCluster>();

            float cameraClusterRange = 10.0f;
             clusterCenter = hdCamera.transform.position;
            ...
            minClusterPos.x = minClusterPos.x < clusterCenter.x - cameraClusterRange? clusterCenter.x - cameraClusterRange : minClusterPos.x;
            minClusterPos.y = minClusterPos.y < clusterCenter.y - cameraClusterRange? clusterCenter.y - cameraClusterRange : minClusterPos.y;
            minClusterPos.z = minClusterPos.z < clusterCenter.z - cameraClusterRange? clusterCenter.z - cameraClusterRange : minClusterPos.z;

            maxClusterPos.x = maxClusterPos.x > clusterCenter.x + cameraClusterRange? clusterCenter.x + cameraClusterRange : maxClusterPos.x;
            maxClusterPos.y = maxClusterPos.y > clusterCenter.y + cameraClusterRange? clusterCenter.y + cameraClusterRange : maxClusterPos.y;
            maxClusterPos.z = maxClusterPos.z > clusterCenter.z + cameraClusterRange? clusterCenter.z + cameraClusterRange : maxClusterPos.z;
        ...
        void CullLights(CommandBuffer cmd)
        {
            using (new ProfilingScope(cmd, ProfilingSampler.Get(HDProfileId.RaytracingCullLights)))
            {
                ...
                cmd.SetComputeFloatParam(lightClusterCS, _LightVolumeCount, Asfloat(totalLightCount));
        ...
        void BuildLightData(CommandBuffer cmd, Camera hdCamera, HDRayTracingLights rayTracingLights)
        {
            // If no lights, exit
            ...
            for (int lightIdx = 0; lightIdx < rayTracingLights.hdLightArray.Count; ++lightIdx)
            {
                ...
                processedData.distanceToCamera = (additionalLightData.gameObject.transform.position - hdCamera.transform.position).magnitude;
                processedData.lightDistanceFade = ComputeLinearDistanceFade(processedData.distanceToCamera, additionalLightData.fadeDistance);
                processedData.volumetricDistanceFade = ComputeLinearDistanceFade(processedData.distanceToCamera, additionalLightData.volumetricFadeDistance);
                ...
                m_RenderPipeline.GetLightData(cmd, visibleLight, in processedData,ref lightDimensions, ref lightData);
        ...
        public void CullForRayTracing(Camera hdCamera, HDRayTracingLights rayTracingLights)
        {
        ...
        public void BuildLightClusterBuffer(CommandBuffer cmd, HDRayTracingLights rayTracingLights)
        {
            // If there is no lights to process or no environment not the shader is missing
            if (totalLightCount == 0 || rayTracingLights.lightCount == 0)
                return;
        ...
        public void BuildRayTracingLightData(CommandBuffer cmd, Camera hdCamera, HDRayTracingLights rayTracingLights)
        {
